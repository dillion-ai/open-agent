...
const XLSX_INSTRUCTIONS = `# Requirements for Outputs
...
const BUILTIN_SKILLS: Record<BuiltInSkillName, Skill> = {
  xlsx: {
    name: 'xlsx',
    description: XLSX_DESCRIPTION,
    instructions: XLSX_INSTRUCTIONS,
  },
};
...
export function loadSkills(
  ...
  // Built-in skills (inlined in code, no file I/O)
  const builtinNames =
    builtins === true
      ? [...BUILTIN_SKILL_NAMES]
      : builtins === false
  ...
  // Load custom user skills from files (raw content, no frontmatter parsing)
  for (const p of customPaths) {
    ...
    if (stat.isDirectory()) {
      ...
      for (const file of files) {
        const skill = loadSkillFile(path.join(resolved, file));
        if (skill) skills.push(skill);
      }
...
function loadSkillFile(filePath: string): Skill | null {
  try {
    ...
    console.warn(
      `[open-agent] Failed to load skill file: ${filePath}`,
